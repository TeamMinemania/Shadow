...
public class FlingCmd extends Command {
    public FlingCmd() {
        super("fling", "flings players in your render distance");
    }
    ...
    public void call(String[] args) {
        ...
        if (args.length < 1) {
            ChatUtils.message("Incorrect usage, please use >fling <player>");
            return;
        }
        ...
        NbtCompound tag = CreativeUtils.parse("{EntityTag:{id:\"minecraft:area_effect_cloud\",Particle:\"block air\",Radius:4.5f,Pos:[" + target.getX() + "," + (target.getY() + 1) + "," + target.getZ() + ",],RadiusPerTick:0f,RadiusOnUse:0f,Duration:40,DurationOnUse:-999f,Age:0,WaitTime:0,Effects:[{Id:25b,Amplifier:100b,Duration:199980,ShowParticles:0b}]}}");
        stack.setNbt(tag);
        ...
        ChatUtils.message("Flung Player " + plr);
    }
}

...
public class NbtPlugin implements NbtElementVisitor {
    ...
    public NbtPlugin(String prefix, int indentationLevel, List<String> pathParts) {
        this.prefix = prefix;
    ...
    public void visitList(NbtList element) {
        if (element.isEmpty()) {
            ...
            for (int i = 0; i < element.size(); ++i) {
                ...
                stringBuilder.append((new NbtPlugin(string, this.indentationLevel + 1, this.pathParts)).apply(element.get(i)));
                if (i != element.size() - 1) {
        ...
        if (compound.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                stringBuilder.append(Strings.repeat(string, this.indentationLevel + 1)).append(escapeName(string2), NAME_COLOR).append(KEY_VALUE_SEPARATOR).append(" ")
                        .append((new NbtPlugin(string, this.indentationLevel + 1, this.pathParts)).apply(Objects.requireNonNull(nbtElement)));
                this.popPathPart();
                if (iterator.hasNext()) {
                    stringBuilder.append(ENTRY_SEPARATOR).append(string.isEmpty() ? " " : "\n");
                }

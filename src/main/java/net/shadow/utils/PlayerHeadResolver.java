...
public class PlayerHeadResolver {
    ...
    public static void resolve(UUID uuid, Texture texture) {
        if (imageCache.containsKey(uuid)) {
            Shadow.c.execute(() -> Shadow.c.getTextureManager().registerTexture(texture, imageCache.get(uuid)));
            return;
        }
        imageCache.put(uuid, EMPTY);
        Shadow.c.execute(() -> Shadow.c.getTextureManager().registerTexture(texture, EMPTY));
        ...
        client.sendAsync(hr, HttpResponse.BodyHandlers.ofByteArray()).thenAccept(httpResponse -> {
            try {
                ...
                Shadow.c.execute(() -> Shadow.c.getTextureManager().registerTexture(texture, nib));
